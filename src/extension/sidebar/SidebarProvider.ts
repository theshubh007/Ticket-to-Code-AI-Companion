...
import { MessageFromWebview, MessageToWebview, CodeChunk, TicketData } from '../types';
import { openFileAtRange, getWorkspaceRoot } from '../utils/editorNavigation';
import { buildContext, deduplicateChunks, boostByFileType } from '../utils/contextManager';
...
export class SidebarProvider implements vscode.WebviewViewProvider {
  ...
  private async _handleAnalyzeRepo(ticketDescription: string): Promise<void> {
    ...
    try {
      ...
      const { chunks } = buildContext(boosted, this._lastTicket ?? {
        key: '', summary: '', description: ticketDescription,
        acceptanceCriteria: '', status: '', priority: '', labels: [], issueType: '',
      });
  ...
  private async _handleGenerateGuide(): Promise<void> {
    if (!this._lastTicket || !this._lastChunks) {
      this._post({ command: 'guideError', payload: 'Fetch a ticket and analyze the repo first.' });
      return;
    }

    try {
      const guide = await this._aiEngine.generateGuide(this._lastTicket, this._lastChunks);
